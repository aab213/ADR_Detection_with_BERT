...
# - pip install https://github.com/kpu/kenlm/archive/master.zip (requires  Microsoft Visual C++ 14.0)
# - pip install gensim
#
# Notes:
# - Had to convert abbreviations_dict.json to a .pkl file
# - Comment out self.ext_vocab2 = self.load_obj('vocabulary_spelling_unique')
#   since it's not used anywhere and the file isn't in the GitHub
# - Comment out self.drug_norm_dict = self.load_obj ('drug_normalize_dict')
# - Download 'punkt' and 'averaged_perceptron_tagger' from nltk for test script
# - Added remove_hashtag function to strip the # symbol but leave the word
# - Added in DrugNorm step
...
class Normalizer ():
    ...
    def load_files(self):
        # AAB - commenting out since the file isn't included and this variable isn't used anywhere
...
#---------Remove URls, email addresses and personal pronouns ------------------
    ...
    def replace_handles(self,list_of_msgs):
        list_of_msgs2 = []
        for msg in list_of_msgs:
            nw_msg = re.sub(r'@\w*', '-TH-', msg)
...
#             nw_msg3 = re.sub(r"(@ [a-zA-Z0-9]+[. ])", ' ', nw_msg2) #remove usernames
    ...
    def remove_registered_icon (self, msg):
        nw_msg = re.sub ('\u00AE', ' ', msg) # AAB: updated to be a space instead of nothing
    ...
    def anonymize (self, posts):
        posts1 = self.replace_handles(posts)
        posts2 = self.replace_urls (posts1)
...
#-------Spelling correction -------------------------------------------------
    ...
    def run_low_emb (self, word, voc, model, w1 =0.6, w2= 0.4):
        replacement = [' ',100]
        for token in voc:
    ...
    def spelling_correction (self, post, min_rel_freq = 9, max_flev_rel = 0.6):
        post2 = []
        cnt = 0
...
#                         self.replaced_with.append(correct)
    ...
    def load_model (self):
        filename = 'file://C://Users//amybr//Desktop//W266_NLP//W266_final_project//LexNorm//obj_lex//Health_2.5mreviews.s200.w10.n5.v15.cbow.bin'
    ...
    def correct_spelling_mistakes(self, data):
        ...
        out = [self.spelling_correction(m) for m in data if m]
...
#--------Overall normalization function--------------------------------------
    ...
    def normalize (self, posts):
        ...
        posts1 = [m.lower() for m in posts0]
        posts2 = self.anonymize(posts1)
        #print(posts1[0])
        ...
        posts5 = [self.remove_hashtag(m) for m in posts4]

        posts6 = [word_tokenize(m) for m in posts5]
        #print('done with tokenizing')
        #print(posts5[0])

        posts7 = [self.expandContractions_second(m) for m in posts6]
        #print(posts6[0])

        self.load_ngrammodel()
        posts8 = [self.sarker_normalize(posts7)]
        posts9 = [self.short_abbr_expansion(m) for m in posts8[0]]
        posts10 = [self.expand_abbr(posts9, self.abbr_dict)]
        return posts10[0]
		
#--------Drug normalization + handle removal + lowercase + hashtag function--------------------------------------

    def drug_normalize (self, posts):
        self.load_files ()
        posts0 = [str(m) for m in posts]
        posts1 = [m.lower() for m in posts0]
        posts2 = self.anonymize(posts1)
        posts3 = [self.remove_hashtag(m) for m in posts2]
        posts4 = [word_tokenize(m) for m in posts3]
        posts5 = [DrugNorm().drug_normalize(posts4)]
        return posts5[0]
